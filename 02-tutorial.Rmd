# A tutorial on how to use the EMU-SDMS [^1]{#chap:tutorial}

<!-- [^1]: Some examples of this chapter are adapted versions of examples of the `emuR_intro` vignette. -->
...
<!-- <!-- \begin{figure}[hpt] --> -->
<!-- <!-- \centering --> -->
<!-- <!-- \input{pics/emuSdmsBirdsEye} --> -->
<!-- <!-- \label{fig:queryEngineWorkFlow} --> -->
<!-- <!-- \end{figure} --> -->

<!-- ![(\#fig:queryEngineWorkFlow)](pics/emuSdmsBirdsEye.png) -->
...
<!-- ```{r, rexample:tutorial_create_emuRdemoData, results='show', message=FALSE, rexample=TRUE} -->
<!-- # load the package -->
<!-- library(emuR) -->
...
<!-- <!-- \begin{figure}[hpt] --> -->
<!-- <!-- \centering --> -->
<!-- <!-- \includegraphics[scale=0.7]{pics/msajc003_praat} --> -->
<!-- <!-- \caption{TextGrid annotation of the `emuR_demoData/TextGrid_collection/\allowbreak msajc003.wav` / `.TextGrid` file pair containing the tiers (from top to bottom): *Utterance*, *Intonational*, *Intermediate*, *Word*, *Accent*, *Text*, *Syllable*, *Phoneme*, *Phonetic*, *Tone*, *Foot*.} --> -->
<!-- <!-- \label{fig:msajc003_praatTG} --> -->
<!-- <!-- \end{figure} --> -->
...
<!-- ```{r, rexample:tutorial_tgconv, results='hide', message=FALSE, rexample=TRUE} -->
...
<!-- ```{r, rexample:tutorial_loadEmuDB, results='hide', message=FALSE, rexample=TRUE} -->
...
<!-- <<rexample:tutorial_summary, rexample=TRUE, tidy.opts=list(width.cutoff = 30)>>= -->
<!-- # show summary -->
<!-- summary(dbHandle) -->
<!-- @ -->
...
<!-- ```{r, rexample:tutorial_serve, eval=FALSE, rexample=TRUE} -->
...
<!-- <!-- \centering --> -->
<!-- <!-- \includegraphics[width=1\textwidth]{pics/tutorialEmuWebAppMyFirst} --> -->
<!-- <!-- \caption{Screenshot of `EMU-webApp` displaying `msajc003` bundle of *myFirst* `emuDB`.} --> -->
<!-- <!-- \label{fig:tutorial_emuWebAppMyFirst} --> -->
<!-- <!-- \end{sidewaysfigure} --> -->
...
<!-- ```{r, rexample:tutorial_simpleQuery, rexample=TRUE} -->
...
<!-- ```{r, tutorial_violentlyHier, fig.cap = "Example of a hierarchical annotation of the word *violently* belonging to the *msajc012* bundle of the *myFirst* demo `emuDB`.", echo=FALSE, fig.width=4.5, fig.height=3} -->
<!-- library(ggplot2) -->
...
<!-- ```{r, rexample:tutorial_autobuild, results='hide', message=FALSE, rexample=TRUE} -->
...
<!-- <!-- \begin{figure}[hpt] --> -->
<!-- <!-- \centering --> -->
<!-- <!-- \input{pics/tut_simpleAnnotStruct} --> -->
<!-- <!-- \caption{Schematic annotation structure of the `emuDB` after calling the autobuild function in R Example \@ref(rexample:tutorial_autobuild).} --> -->
<!-- <!-- \label{fig:tutorial_simpleAnnotStruct} --> -->
<!-- <!-- \end{figure} --> -->
...
<!-- ```{r, rexample:tutorial_delBackupLevels, rexample=TRUE} -->
...
<!-- <!-- \begin{sidewaysfigure}[hpt] --> -->
<!-- <!-- \centering --> -->
<!-- <!-- \includegraphics[width=1\textwidth]{pics/EMU-webAppScreenshotTutorialPostAutobHier.png} --> -->
<!-- <!-- \caption{Screenshot of `EMU-webApp` displaying the autobuilt hierarchy of the *myFirst* `emuDB`.} --> -->
<!-- <!-- \label{fig:tutorial_EMU-webAppScreenshotTutorialPostAutobHier} --> -->
<!-- <!-- \end{sidewaysfigure} --> -->
...
<!-- Having this hierarchical annotation structure now allows us to formulate a query that helps answer the originally stated question: *Given an annotated speech database, is vowel height (measured by its correlate, the first formant frequency) influenced by whether it appears in a strong or weak syllable?*. As stated in the question, we are interested in all the vowels in our database. R Example \@ref(rexample:tutorial_labelGroupQuery) shows how all the vowels in the *myFirst* database are queried. -->

<!-- ```{r, rexample:tutorial_labelGroupQuery, rexample=TRUE} -->
...
<!-- sl_vowels = query(dbHandle, paste0("Phonetic == A | E | EC ", -->
<!--                                    "| I | V | U | ai | ei | oi ", -->
<!--                                    "| i@ | u@ | au | @u | @: ", -->
<!--                                    "| @ | '=' | a: | e: | i: ", -->
<!--                                    "| o: | u:")) -->
...
<!-- ```{r, rexample:tutorial_requery, rexample=TRUE} -->
...
<!-- ```{r, rexample:tutorial_getTrackdata, rexample=TRUE} -->
<!-- # get formant values for -->
...
<!-- As can be seen by the call to the `class()` function, the resulting object is of the type `trackdata` and has \Sexpr{nrow(sl_vowels)} entries. This corresponds to the number of rows contained in the segment lists extracted above (i.e., `nrow(sl_vowels)`). This indicates that this object contains data for each of the segments that correspond to each of the row entries of the segment lists (i.e., `td_vowels[5,]` are the formant values belonging to `sl_vowels[5,]`). As the columns `T1`, `T2`, `T3`, `T4` of the printed output of `td_vowels[5,]` suggest, the `forest` function estimates four formant values. We will only be concerned with the first (column `T1`) and second (column `T2`). R Example \@ref(rexample:tutorial_dplot) shows a call to `emuDB`'s `dplot()` function which produces the plot displayed in Figure \@ref(fig:tutorial_dplot). The first call to the `dplot()` function plots all \Sexpr{nrow(sl_vowels)} first formant trajectories (achieved by indexing the first column i.e., `T1`: `x = td_vowels[, 1]`). The \Sexpr{nrow(sl_vowels)} trajectories are color coded by vowel label (`labs = sl_vowels\$labels`). To clean up the cluttered left plot, the second call to the `dplot()` function additionally uses the `average` parameter to plot only the ensemble averages of each vowel label and time-normalizes the trajectories (`normalise = TRUE`) to an interval between 0 and 1. -->

<!-- ```{r, rexample:tutorial_dplot, rexample=TRUE, eval=FALSE} -->
...
<!-- ```{r, tutorial_dplot, fig.cap = "`dplot()` plots of F1 trajectories. The left plot displays 81 trajectories while the right plot displays ensemble averages of each vowel.", echo=FALSE, fig.width=6, fig.height=6} -->
<!-- par(mfrow = c(1,2)) -->
...
<!-- Figure \@ref(fig:tutorial_dplot) gives an overview of the second formant trajectories by vowel class. For the purpose of data exploration and to get an idea of where the individual vowel classes lie on the F2 x F1 plane, which indirectly provides information about vowel height and tongue position, R Example \@ref(rexample:tutorial_eplot) makes use of the `eplot()` function. This produces Figure \@ref(fig:tutorial_eplot). To be able to use the `eplot()` function, the `td_vowels` object first has to be modified, as it contains entire formant trajectories but two dimensional data is needed to be able to display it on the F2 x F1 plain. This can, for example, be achieved by only extracting temporal mid-point formant values for each vowel using the `get_trackdata()` function utilizing its `cut` parameter. R Example \@ref(rexample:tutorial_eplot) shows an alternative approach using the `dcut()` function to essentially cut the formant trajectories to a specified proportional segment. By using only the `left.time = 0.5` (and not specifying `right.time`) only the formant values that are closest to the temporal mid-point are cut from the trajectories. -->

<!-- ```{r, rexample:tutorial_eplot, rexample=TRUE, eval=FALSE} -->
...
<!-- Figure \@ref(fig:tutorial_eplot) displays the first two formants extracted at the temporal midpoint of every vowel in `sl_vowels`. These formants are plotted on the F2 x F1 plane, and their 95% ellipsis distributions are also shown. Although not necessarily applicable to the question posed at the beginning of this tutorial, the data exploration using the `dplot()` and `eplot()` functions can be very helpful tools for providing an overview of the data at hand. -->


<!-- ## Vowel height as a function of syllable types (strong vs. weak): evaluation and statistical analysis -->

<!-- The above data exploration only dealt with the actual vowels and disregarded the syllable type they occurred in. However, the question in the introduction of this chapter does not distinguish between vowel classes but focuses on whether a vowel occurs in a strong or weak syllable. For the sake of this tutorial we will disregard the fact that the vowel class influences the syllable type and only focus on whether a vowel occurred in a syllable labeled *S* (strong) or *W* (weak). For data inspection purposes, R Example \@ref(rexample:tutorial_dplotSylTyp) initially extracts the central 60% (`left.time = 0.2` and `right.time = 0.8`) of the formant trajectories from `td_vowels` using `dcut()` and displays them using `dplot()`. It should be noted that the call to `dplot()` uses the labels of the `sl_sylType` object as opposed to those of `sl_vowels`. This causes the `dplot()` functions to group the trajectories by their syllable type as opposed to their vowel class as displayed in Figure \@ref(fig:tutorial_dplotSylTyp). -->


<!-- ```{r, rexample:tutorial_dplotSylTyp, eval=FALSE, rexample=TRUE} -->
...
<!-- dplot(x = td_vowelsMidSec[, 1], -->
<!--       labs = sl_sylType$labels, -->
<!--       normalise = TRUE, -->
<!--       average = TRUE, -->
<!--       xlab = "Normalized time", -->
<!--       ylab = "F1 (Hz)") -->
...
<!-- As can be seen in Figure \@ref(fig:tutorial_dplotSylTyp), there seems to be a distinction in F1 trajectory height between vowels in strong syllables and weak syllables. R Example \@ref(rexample:tutorial_boxplot) shows the code to produce a boxplot using the `ggplot2` package to further visually inspect the data (see Figure \@ref(fig:tutorial_boxplot) for the plot produced by R Example \@ref(rexample:tutorial_boxplot)). -->



<!-- ```{r, rexample:tutorial_boxplot, rexample=TRUE, eval=FALSE} -->
<!-- # use trapply to calculate the means of the 60% -->
<!-- # formant trajectories -->
<!-- td_vowelsMidSec_mean = trapply(td_vowelsMidSec[, 1], -->
<!--                                fun = mean, -->
<!--                                simplify = T) -->

<!-- # create new data frame that contains the mean -->
<!-- # values and the corresponding labels -->
<!-- df = data.frame(sylType = sl_sylType$labels, -->
...
<!-- ggplot(df, -->
<!--        aes(sylType, meanF1)) + geom_boxplot() -->

<!-- ``` -->

<!-- ```{r, tutorial_boxplot, fig.cap = "Boxplot produced using `ggplot2` to visualize the difference in F1 depending on whether the vowel occurs in strong (S) or weak (W) syllables.", echo=FALSE, fig.width=6, fig.height=3} -->
<!-- td_vowelsMidSec_mean = trapply(td_vowelsMidSec[, 1], -->
<!--                                fun = mean, -->
<!--                                simplify = T) -->
...
<!-- To confirm this, R Example \@ref(rexample:tutorial_stats1) presents a simple statistical analysis of the F1 values. First, a Shapiro-Wilk test for normality of the distributions of the F1 for both syllable types is carried out. As neither syllable type is normally distributed, the test for normality is negative in both cases, and a Wilcoxon rank sum test is performed. The density distributions (commented out `plot()` function calls in R Example \@ref(rexample:tutorial_stats1)) are displayed in Figure \@ref(fig:tutorial_stats1). -->

<!-- ```{r, rexample:tutorial_stats1, rexample=TRUE} -->
<!-- # replace vowel labels with labels of the -->
<!-- # syllable type they belong to in sl_vowels -->
<!-- # to have the correct start and end time -->
<!-- # as well as label information in a single -->
<!-- # segment list -->
<!-- sl_vowels$labels = sl_sylType$labels -->

<!-- # use create_emuRtrackdata function -->
<!-- # to create a data.table object that -->
<!-- # contains all the information of both -->
<!-- # the segment list and trackdata object -->
<!-- vowelDB = create_emuRtrackdata(sl_vowels, td_vowels) -->

<!-- # calculate density -->
<!-- distrS = density(vowelDB[vowelDB$labels=="S",]$T1) -->

<!-- # uncomment to visualize distribution -->
<!-- # plot(distrS) -->

<!-- # check that vowels in strong -->
<!-- # syllables are normally distributed: -->
<!-- shapiro.test(vowelDB[vowelDB$labels=="S",]$T1) -->

<!-- # no, use Wilcoxon rank sum test. -->

<!-- # calculate density -->
<!-- distrW = density(vowelDB[vowelDB$labels=="W",]$T1) -->

<!-- # uncomment to visualize distribution -->
<!-- # plot(distrW) # also doesn't look like it -->

<!-- # check that vowels in weak -->
<!-- # syllables are normally distributed -->
<!-- shapiro.test(vowelDB[vowelDB$labels=="W",]$T1) -->

<!-- # no, use Wilcoxon rank sum test. -->

<!-- # perform Wilcoxon rank sum test to establish -->
<!-- # whether vowel F1 depends on syllable type -->
<!-- wilcox.test(T1 ~ labels, data=vowelDB) -->
<!-- ``` -->

<!-- ```{r, tutorial_stats1, fig.cap = "Plots of density distributions of vowels in strong syllables (left plot) and vowels in weak syllables (right plot) in R Example \\ref{rexample:tutorial_stats1}.", echo=FALSE, fig.width=6, fig.height=3} -->
<!-- # two plots next to each other -->
<!-- par(mfrow = c(1,2)) -->
...
<!-- As shown by the result of `wilcox.test()` in R Example \@ref(rexample:tutorial_stats1), syllable type (*S* vs. *W*}) has a significant influence on the vowel's F1 (W=271210, p<0.001). Hence, the answer to the initially proposed question: *Given an annotated speech database, is vowel height (measured by its correlate, the first formant frequency) influenced by whether it appears in a strong or weak syllable?* is yes! -->
