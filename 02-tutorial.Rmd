...
# show head of td_vowels
head(td_vowels, n = 1)

```

As can be seen by the call to the `class()` function, the resulting object is of the type `emuRtrackdata` and has `r nrow(td_vowels)` rows. The `td_vowels$sl_rowIdx` column of the `td_vowels` object is vital as it indicates which row of `td_vowels` belongs to which row in `sl_vowels`. As the columns `T1`, `T2`, `T3`, `T4` of the printed output of `head(td_vowels, n = 1)` suggest, the `forest` function estimates four formant values. We will only be concerned with the first (column `T1`) and second (column `T2`). R Example \@ref(rexample:tutorial_dplot) shows a call to `ggplot()` which produces the plot displayed in Figure \@ref(fig:tutorial_dplot). The call to the `ggplot()` function plots all `r nrow(sl_vowels)` first formant trajectories (achieved by `group = sl_rowIdx`). The `r nrow(sl_vowels)` trajectories are color coded by vowel label (`col = labels`).
