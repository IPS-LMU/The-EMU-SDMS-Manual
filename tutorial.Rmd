...
```{r tutorial-emuWebAppMyFirst, fig.cap="Screenshot of `EMU-webApp` displaying `msajc003` bundle of *my-first* `emuDB`.", fig.align="center", echo=FALSE, out.width = "100%"}
...
```{r tutorial-violentlyHier, fig.cap = "Example of a hierarchical annotation of the content (==*C*) word *violently* belonging to the *msajc012* bundle of the *my-first* demo `emuDB`.", fig.align="center", echo=FALSE, fig.width=4.5, fig.height=3}
library(ggplot2)
...
```{r tutorial-EMU-webAppScreenshotTutorialPostAutobHier, fig.cap="Screenshot of `EMU-webApp` displaying the autobuilt hierarchy of the *my-first* `emuDB`.", fig.align="center", echo=FALSE, out.width = "100%"}
...
As can be seen by the call to the `class()` function, the resulting object is of the type `tibble` (see `?tibble::tibble` for more information) and has `r max(td_vowels$sl_rowIdx)` blocks of data. These blocks correspond to the number of rows contained in the segment lists extracted above (i.e., `nrow(sl_vowels)`) and can be matched to the according segment using the `sl_rowIdx` column (i.e., `td_vowels %>% filter(sl_rowIdx == 5)` are the formant values belonging to `sl_vowels[5,]`). As the columns `T1`, `T2`, `T3`, `T4` of the printed output of `td_vowels %>% filter(sl_rowIdx == 5)` suggest, the `forest` function estimates four formant values. We will only be concerned with the first (column `T1`) and second (column `T2`). The below R code shows two `ggplot()` function calls which produce the plots displayed in Figures \@ref(fig:tutorial-dplot1) and \@ref(fig:tutorial-dplot1). The first `ggplot()` call plots all `r nrow(sl_vowels)` first formant trajectories (achieved by setting the `group` parameter to `sl_rowIdx`). To clean up the cluttered first plot, the second `ggplot()` call uses a segment length normalized version of `td_vowels` (see `?normalize_length` for futher details) as well as using `geom_smooth()` in combination with setting the `group` parameter to `labels` to plot only smoothed conditional means of all *\@* vowels.
...
```{r tutorial-dplot1, fig.cap = "`ggplot()` plots of all F1 *\\@* vowel trajectories.", fig.align="center", echo=FALSE, fig.width=6, fig.height=6, message=F}
# load package
library(ggplot2)
...
```{r tutorial-dplot2, fig.cap = "`ggplot()` plots of the F1 smoothed conditional mean trajectories of all *\\@* vowels.", fig.align="center", echo=FALSE, fig.width=6, fig.height=6, message=F, warning=F}
# normalize length of segments
td_vowels_norm = normalize_length(td_vowels)
...
```{r tutorial-eplot, message=FALSE, fig.cap = "95% ellipse plot including centroid for F2 x F1 data extracted from the temporal midpoint of the vowel segments.", fig.align="center", echo=FALSE, fig.width=6, fig.height=6}
# cut formant trajectories at temporal mid-point
td_vowels_midpoint = td_vowels_norm %>% 
  filter(times_norm == 0.5)
