...
```{r, rexample:webApp_serve, rexample=TRUE, eval=FALSE}
...
R Example \ref{rexample:webApp_loadDemoData} shows how to create and load the demo data that will be used throughout the rest of this chapter.


```{r, rexample:webApp_loadDemoData, rexample=TRUE, results='hide', message=FALSE}
# load package
library(emuR)
...
As mentioned above, the EMU-webApp subdivides different ways to look at an emuDB into so-called `perspective`s. Users can switch between these `perspective`s in the web application. They contain, for example, information on what levels are displayed, which SSFF tracks are drawn. R Example \ref{rexample:webApp_listPersp} shows how the current `perspective`s can be listed using the `list_perspectives()` function.


```{r, rexample:webApp_listPersp, rexample=TRUE}
# list perspectives of ae emuDB
list_perspectives(ae)
```

As it is sometimes necessary to add new or remove existing perspectives to or from a database, R Example \ref{rexample:webApp_addRemovePersp} shows how this can be achieved using emuR's `add/remove_perspective()` functions.

<!-- <<>>= -->
```{r, rexample:webApp_addRemovePersp, rexample=TRUE}
