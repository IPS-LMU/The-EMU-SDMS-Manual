...
```{r rexample:query_loadDemoData, rexample=TRUE, results='hide', message=FALSE}
# load package
library(emuR)
...
```{r rexample:emuRsegs, rexample=TRUE}
# query database
...
```{r rexample:simpleq, rexample=TRUE}
...
```{r rexample:simpleq-deduceTime, rexample=TRUE}
# query all annotation items containing
...
```{r rexample:resultmod, rexample=TRUE}
...
```{r rexample:conjq, rexample=TRUE}
...
```{r amongstHierDomintation, fig.cap="Partial hierarchy depicting all annotation items that are dominated by the strong syllable (*S*) of the *Syllable* level (inside dashed box). Items marked \textcolor{three_color_c1}{green} belong to the \textcolor{three_color_c1}{*Phoneme*} level, items marked \textcolor{three_color_c2}{orange} belong to the \textcolor{three_color_c2}{*Phonetic*} level and the \textcolor{three_color_c3}{purple} dashed box indicates the set of items that are dominated by *S*.", fig.align="center", echo=FALSE, out.width = "75%"}
...
```{r query-positionSimple, fig.cap="Illustration of what is returned by the `Start()`, `Medial()` and `End()` functions depending if they are set to \textbf{A:} \textcolor{three_color_c1}{`TRUE`} (green) or \textbf{B:} \textcolor{three_color_c2}{`FALSE`} (orange).", fig.align="center", echo=FALSE, out.width = "75%"}
knitr::include_graphics("pics/positionSimple.png")
```

```{r rexample:posq, rexample=TRUE}
...
```{r rexample:query-countQuery, rexample=TRUE}
...
```{r rexample:query-moreComplexQuery, rexample=TRUE}
...
```{r rexample:query-deducingTime, rexample=TRUE}
...
```{r rexample:query-requery, rexample=TRUE}
########################
# requery_seq()
