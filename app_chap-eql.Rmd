...
```{r rexample:app-chap-eqlLoadData, rexample=TRUE, results='hide', message=FALSE}
# load the package 
library(emuR)
...
```{r echo=FALSE}
cat("Can't find the query results in emu.query: there may have \nbeen a problem with the query command.")
```
...
```
```{r echo=FALSE}
cat("segment  list from database:  andosl
query was:  [Text=spring & #Accent=S]
  labels    start      end          utts
...
```{r echo=FALSE}
cat("Error in query.database.eql.KONJA(dbConfig, qTrim) :
...
```{r echo=FALSE}
cat("segment  list from database:  ae
query was:  [Text!=beautiful|futile ^ Phoneme=u:]
  labels    start      end          utts
