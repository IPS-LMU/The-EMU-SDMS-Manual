...
```{r rexample:emuDB-create, rexample=TRUE}
# load package
...
```{r rexample:emuDB-load, rexample=TRUE}
# load database
...
```{r rexample:emuDB-addLevelDefPhonetic, rexample=TRUE}
...
```{r rexample:emuDB-addLevelDefWord, rexample=TRUE}
...
```{r rexample:emuDB-listAttrDef, rexample=TRUE}
...
```{r rexample:emuDB-addAttrDef, rexample=TRUE}
# add
...
```{r rexample:emuDB-addLinkDef, rexample=TRUE}
...
```{r emuDB-fromScratchHier, fig.cap="A schematic representation of the simple hierarchical structure of the *fromScratch* created by the `add_linkDefinition()` function call in R Example @ref(rexample:emuDB-addLinkDef).", fig.align="center", echo=FALSE, out.width = "50%"}
...
```{r rexample:emuDB-importMediaFiles, rexample=TRUE}
...
```{r rexample:emuDB-addFiles, rexample=TRUE}
# load wrassp package
library(wrassp)
...
```{r rexample:emuDB-addTrack, rexample=TRUE}
...
```{r rexample:configWebApp, rexample=TRUE}
