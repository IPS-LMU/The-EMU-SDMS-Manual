...
```{r rexample:wrassp-help, rexample=TRUE, eval=FALSE}
help(package="wrassp")
```
...
```{r rexample:wrassp-loadDemoData, rexample=TRUE, message=FALSE}
...
```{r rexample:wrassp-read, rexample=TRUE, tidy=TRUE}
...
```{r rexample:wrassp-extrAttrs, rexample=TRUE}
# show duration
dur.AsspDataObj(au)
...
```{r rexample:wrassp-extrData, rexample=TRUE}
# show track names
...
```{r rexample:wrassp-plotOsci, rexample=TRUE, eval=FALSE}
...
```{r wrassp-plotOsci, fig.cap = "Oscillogram generated from samples stored in the `audio` track of the object `au`.", fig.width=6, fig.height=3, echo=FALSE}
# calculate sample time of every 10th sample
samplesIdx = seq(0, numRecs.AsspDataObj(au) - 1, 10)
samplesTime = samplesIdx / rate.AsspDataObj(au)
...
```{r rexample:wrassp-writeAudio, rexample=TRUE, results='hide'}
...
```{r rexample:wrassp-outputInfosName, rexample=TRUE}
...
```{r rexample:wrassp-outputInfosForest, rexample=TRUE}
...
```{r rexample:wrassp-calcFms, rexample=TRUE}
...
```{r rexample:wrassp-plotFms, rexample=TRUE, eval=FALSE}
...
```{r wrassp-plotFms, fig.cap = "Matrix plot of formant values stored in the `fm` track of `fmBwVals` object.", fig.width=6, fig.height=3, echo=FALSE}
# plot the formant values
matplot(seq(0,numRecs.AsspDataObj(fmBwVals) - 1)
        / rate.AsspDataObj(fmBwVals)
        + attr(fmBwVals, "startTime"),
...
```{r rexample:wrassp-calcF0, rexample=TRUE, results='hide'}
...
```{r rexample:wrassp-plotF0, rexample=TRUE, eval=FALSE}
...
```{r wrassp-plotF0, fig.cap = "Plot of fundamental frequency values stored in the `F0` track of `f0vals` object.", fig.width=6, fig.height=3, echo=FALSE}
# plot the fundamental frequency contour
plot(seq(0,numRecs.AsspDataObj(f0vals) - 1)
     / rate.AsspDataObj(f0vals) +
       attr(f0vals, "startTime"),
...
```{r rexample:wrassp-calcRMS, rexample=TRUE, results='hide', message=FALSE}
...
```{r rexample:wrassp-plotRMS, rexample=TRUE, eval=FALSE}
...
```{r wrassp-plotRMS, fig.cap = "Plot of RMS values stored in `rms` track of the `rmsvals` object.", fig.width=6, fig.height=3, echo=FALSE}
# plot the RMS energy contour
plot(seq(0, numRecs.AsspDataObj(rmsvals) - 1)
     / rate.AsspDataObj(rmsvals)
     + attr(rmsvals, "startTime"),
...
```{r rexample:wrassp-logging, rexample=TRUE, tidy=TRUE, warning=FALSE}
...
```{r rexample:wrassp-addSSFF, rexample=TRUE, results='hide'}
# load emuDB
...
```{r rexample:wrassp-onTheFly, rexample=TRUE}
...
```{r rexample:wrassp-genSin, rexample=TRUE, eval=FALSE}
...
```{r wrassp-genSin, fig.cap = "A single cycle sine wave consisting of 16000 samples.", fig.width=6, fig.height=3, echo=FALSE}
x = seq(0, 2 * pi, length.out = 16000)
sineWave = sin(x)
plot(x, sineWave, type = 'l',
     xlab = "x from 0 to 2*pi",
...
```{r rexample:wrassp-genAdo, rexample=TRUE}
